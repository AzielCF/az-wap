                        ...
                        <tr v-for="ch in channels" :key="ch.id" class="hover:bg-white/[0.03] transition-all border-b border-white/5 group">
                            <td class="py-6 pl-10 align-middle">
                                <div class="flex items-center gap-5">
                                    ...
                                    <div class="icon-box-premium w-14 h-14 bg-black/40 ring-1 ring-white/5 flex-shrink-0" v-else-if="ch.type === 'telegram'">
                                        <Send class="w-8 h-8 text-[#0088cc]" />
                                    </div>
          ...
          <div class="flex-1 overflow-y-auto p-12 custom-scrollbar space-y-12 bg-[#0b0e14]">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-6 sm:gap-8 p-6 sm:p-10 bg-white/[0.02] rounded-[2rem] border border-white/5 text-center sm:text-left">
                <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-[1.5rem] bg-black/40 flex items-center justify-center ring-1 ring-white/10 shadow-2xl flex-none">
                    <svg v-if="newChannel.type === 'whatsapp'" class="w-8 h-8 sm:w-10 sm:h-10 text-[#25D366]" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                    </svg>
                    <Send v-else class="w-8 h-8 sm:w-10 sm:h-10 text-[#0088cc]" />
                </div>
                <div class="flex-1">
                    <h3 class="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter mb-1 leading-none">
                        {{ newChannel.type === 'whatsapp' ? 'WhatsApp Web Gateway' : 'Telegram Bot API' }}
                    </h3>
                    <p class="text-xs sm:text-xs text-slate-600 font-bold uppercase tracking-widest">
                        {{ newChannel.type === 'whatsapp' ? 'Connect your account using QR Code' : 'Connect your bot using API Token' }}
                    </p>
                </div>
            </div>
            ...
            <div class="space-y-8">
                <div class="form-control w-full">
                    ...
                    <input v-model="newChannel.name" type="text" placeholder="e.g. Sales Bot" class="input-premium h-14 sm:h-16 w-full text-base sm:text-lg font-black" />
                ...
                <div class="form-control w-full">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <button 
                            @click="newChannel.type = 'whatsapp'"
                            class="p-4 sm:p-6 bg-white/[0.03] rounded-2xl border transition-all flex items-center gap-4 group cursor-pointer"
                            :class="newChannel.type === 'whatsapp' ? 'border-primary bg-primary/5 shadow-lg shadow-primary/10' : 'border-white/5 hover:border-white/10'"
                        >
                            ...
                            <span class="text-xs font-black text-white uppercase tracking-widest leading-none">WhatsApp</span>
                        </button>
                        <button 
                            @click="newChannel.type = 'telegram'"
                            class="p-4 sm:p-6 bg-white/[0.03] rounded-2xl border transition-all flex items-center gap-4 group cursor-pointer"
                            :class="newChannel.type === 'telegram' ? 'border-primary bg-primary/5 shadow-lg shadow-primary/10' : 'border-white/5 hover:border-white/10'"
                        >
                            <div class="w-10 h-10 rounded-xl bg-[#0088cc]/10 flex items-center justify-center text-[#0088cc] flex-none">
                                <Send class="w-5 h-5" />
                            </div>
                            <span class="text-xs font-black text-white uppercase tracking-widest leading-none">Telegram</span>
                        </button>
                    </div>
                </div>
            </div>

            <!-- Warning Box / Token Field -->
            <div v-if="newChannel.type === 'whatsapp'" class="p-6 sm:p-8 bg-amber-500/5 border border-amber-500/10 rounded-2xl flex flex-col sm:flex-row gap-4 sm:gap-6">
            ...
            <div v-else class="space-y-4">
                <div class="form-control w-full">
                    <label class="label-premium text-xs text-primary">Bot Father Token</label>
                    <input v-model="(newChannel as any).token" type="password" placeholder="123456789:ABCdefGHI..." class="input-premium h-14 w-full font-mono text-sm" />
                    <p class="mt-2 text-xs font-bold text-slate-700 uppercase tracking-tight">Enter the token provided by @BotFather</p>
                </div>
            </div>
          </div>
