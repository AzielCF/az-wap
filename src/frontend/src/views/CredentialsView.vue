  ...
  <div v-else class="space-y-12 animate-in fade-in duration-700 max-w-[1400px] mx-auto pb-20">
    ...
    <AppPageHeader title="Credentials">
      <template #breadcrumb>
          ...
          <span class="text-sm font-bold uppercase tracking-widest text-primary">Security Vault</span>
          <span class="opacity-30 text-xs font-black text-slate-500">/</span>
          <span class="text-xs font-bold uppercase tracking-widest text-slate-500">Global Credentials</span>
      </template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 lg:px-0">
        <div class="p-8 bg-[#161a23]/30 border border-white/5 rounded-[2rem] space-y-2">
            <span class="text-xs font-bold text-slate-600 uppercase tracking-widest">Total Keys</span>
        ...
        <div class="p-8 bg-[#161a23]/30 border border-white/5 rounded-[2rem] space-y-2">
            <span class="text-xs font-bold text-slate-600 uppercase tracking-widest">AI Providers</span>
            <div class="flex items-baseline gap-2">
        ...
        <div class="p-8 bg-[#161a23]/30 border border-white/5 rounded-[2rem] space-y-2">
            <span class="text-xs font-bold text-slate-600 uppercase tracking-widest">External Bridges</span>
            <div class="flex items-baseline gap-2">
    ...
    <div class="px-6 lg:px-0">
        ...
        <div v-if="filteredCredentials.length === 0" class="py-40 flex flex-col items-center justify-center bg-[#161a23]/20 rounded-[3rem] border border-dashed border-white/5">
            ...
            <p class="text-sm font-bold text-slate-600 uppercase tracking-widest">No credentials found in the vault</p>
            ...
            <div v-for="cred in filteredCredentials" :key="cred.id" class="card-premium">
                <div class="relative z-10 flex flex-col h-full">
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <span class="text-xs font-bold text-slate-600 uppercase tracking-widest font-mono">{{ cred.id.substring(0,8) }}</span>
                    </div>

                    <div class="space-y-4 mt-auto">
                        <div v-if="isAI(cred.kind)" class="storage-box-premium">
                            <div class="flex items-baseline justify-between mb-1">
                                <span class="text-xs font-bold text-slate-600 uppercase">API KEY</span>
                        ...
                        <div v-if="cred.kind === 'chatwoot'" class="storage-box-premium space-y-3">
                            <div>
                                <span class="text-xs font-bold text-slate-600 uppercase block mb-1">INSTANCE URL</span>
                                <span class="text-xs font-medium text-slate-400 line-clamp-1 italic">{{ cred.chatwoot_base_url }}</span>
                            </div>
                            <div class="flex items-center justify-between border-t border-white/5 pt-2">
                                <div class="flex items-center gap-2">
                                    <ShieldCheck class="w-3 h-3 text-success/50" />
                                    <span class="text-xs font-black text-slate-600 uppercase">TOKEN VAULTED</span>
                                </div>
                                <span class="text-xs font-black text-slate-700 uppercase">ENCRYPTED</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-black text-slate-500 uppercase tracking-widest">Secure Connection</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-12 custom-scrollbar space-y-10 bg-[#0b0e14]">
                ...
                <div class="space-y-8">
                    <div class="form-control">
                        ...
                        <p class="text-xs text-slate-600 font-bold uppercase tracking-widest mt-2 px-1">Used to identify this key when configuring bots.</p>
                    </div>
